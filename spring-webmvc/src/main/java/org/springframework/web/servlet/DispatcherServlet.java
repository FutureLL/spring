...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		/**
		 * 从配置文件加载默认配置实现
		 * 也就是从这个文件中 DEFAULT_STRATEGIES_PATH 拿到所有配置
		 * 一共8个配置
	...
	protected void initStrategies(ApplicationContext context) {
		// 上传文件的 Bean,这里解释了上传文件需要固定名称: multipartResolver
		initMultipartResolver(context);
		// 国际化的东西,一般很少用
		initLocaleResolver(context);
		// 前端的主题样式,这个其实是利用 Spring 来做的
		initThemeResolver(context);
		// 初始化 HandlerMappings,这个非常重要
		initHandlerMappings(context);
		// 初始化 HandlerAdapters,这个非常重要
		initHandlerAdapters(context);
		// 可以自己提供一个 HandlerException
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		...
		// 经过 static 静态块,defaultStrategies 已经有数据了(8个)
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 确定当前请求的处理程序适配器(适配器: 不同的 Controller 交给不同的适配器(HandlerAdapter)去处理)
				// 如果 HandlerExecutionChain 是一个 Bean, mappedHandler.getHandler() 返回一个对象
				// 如果 HandlerExecutionChain 是一个方法, mappedHandler.getHandler() 返回一个方法
				// 注意: 如果是正常的 Controller,那么拿到的是一个方法
				HandlerAdapter ha = getHandlerAdapter(mappedHandler.getHandler());

				// ----------------------------------------------------------------
				// 到这里 Spring 才确定需要怎么反射调用
	 ...
	 * 	-- 举例: @RequestMapping("/tiger")
	 * 			public class TigerController implements org.springframework.web.servlet.mvc.Controller {}
	 *  @see com.future.mvc.controller.TigerController
	 *
	 * handlerMappings.add(RequestMappingHandlerMapping);
	 * 	-- 举例: @RequestMapping("/index")
	 * 	@see com.future.mvc.controller.IndexController
	 */
	@Nullable
	protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// handlerMappings 是怎么来的?
		// 由 static 静态代码块 set 的,位置: org/springframework/web/servlet/DispatcherServlet.java:283
		// static 中要加载 DispatcherServlet.properties 文件,该文件中对 HandlerMapping 进行初始化
		// org.springframework.web.servlet.HandlerMapping
		// ****************************************************************************************************
		// ** Spring5.0.x 源码中 handlerMappings 中有两个 HandlerMapping, 而 Spring5.1.8 中有五个 handlerMappings **
		// ****************************************************************************************************
		if (this.handlerMappings != null) {
			for (HandlerMapping mapping : this.handlerMappings) {
				// 把请求传过去确定,看能不能的到一个 handler
				// 注意怎么得到一个 handler 和 HandlerMapping 自己实现的逻辑有关
